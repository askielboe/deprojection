...
do n = 1, nspectra
	if (params(n) < 9.999) then
		...
		write(fnamein,'(a,I1,a,F8.3,a)') './data/output/fakespec_',ipar,'-',params(n),'.txt'
	end if
...
write(fnameout,'(a,I1,a,I5,a)') './data/profiles/profile_',ipar,'_',chosenchannel,'.txt'
open(2,file=fnameout, status="replace", form='FORMATTED')

do n = 1,nspectra
	write(2,'(F7.3,a,F10.7)') params(n), ' ', count(n,chosenchannel)/exposure*1000
end do
close(2)

! Print output to file (3-d)
! open(2,file='./data/profiles/profile.txt', status="replace", form='FORMATTED')
! 
...
write(*,'(a,I1,a,I5,a)') 'Output written to: ./data/profiles/profile_',ipar,'_',chosenchannel,'.txt'
